LANGUAGE_NAME := tree-sitter-oracle-plsql
HOMEPAGE_URL := https://github.com/hrushikeshpawar/tree-sitter-oracle_plsql
VERSION := 0.1.0
DESCRIPTION := Tree-Sitter grammar for Oracle PLSQL
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/oracle-plsql '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/oracle-plsql
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/oracle-plsql
